...
func (v *Verifier) VerifyItem(ctx context.Context, item *database.TransferItem, remoteDest string) error {
	logger.For("verifier").WithOp(item.OperationID).Infof("Starting verification for item #%d (%s)", item.ID, item.RelativePath)
	...
	if err != nil {
		...
		_ = v.repo.LogEvent(&item.OperationID, "ERROR", "verifier", "Verification failed", fmt.Sprintf("Item %s: %v", item.RelativePath, err))
		logger.For("verifier").WithOp(item.OperationID).Errorf("Verification failed for item %s: %v", item.RelativePath, err)
		return err
	}
	...
	if res.DifferCount > 0 {
		...
		_ = v.repo.LogEvent(&item.OperationID, "WARN", "verifier", "Verification mismatch", fmt.Sprintf("Item %s has differing files", item.RelativePath))
		logger.For("verifier").WithOp(item.OperationID).Warnf("Verification mismatch for item %s (%d differing files)", item.RelativePath, res.DifferCount)
		return fmt.Errorf("%s", errStr)
	}

	now := time.Now().UTC()
	if err := v.repo.UpdateItemVerified(item.ID, now, "VERIFIED_OK", "VERIFIED_OK"); err != nil {
		logger.For("verifier").WithOp(item.OperationID).Errorf("Failed to update verified status in db for item %s: %v", item.RelativePath, err)
		return fmt.Errorf("failed to update verified status in db: %w", err)
	}

	_ = v.repo.LogEvent(&item.OperationID, "INFO", "verifier", "Item verified successfully", fmt.Sprintf("Item: %s", item.RelativePath))
	logger.For("verifier").WithOp(item.OperationID).Infof("Item verified successfully on Google Drive: %s", item.RelativePath)
	return nil
}
